/**
 * Writing-assist modes (task 12.8) — a direct port of mobile's task 1.4
 * (`apps/mobile/src/chat/modes/modes.ts`). Same fields, same prompts, same
 * temperatures: these were empirically tuned against the smallest catalog
 * model on mobile's own inference path, not re-derived here. Nothing here
 * touches the inference layer — `ChatMessage` already carries a `system`
 * role and `generateChat` already takes a temperature, so a mode is data,
 * not a code path.
 *
...
export type Mode = {
   ...
   *
   * Only plain chat does. This is not a preference — it was measured on
   * mobile. With two prior turns of the model correcting grammar in the
   * transcript, switching to Brainstorm and sending the same text produced
   * *another grammar correction*: a 0.5B model weights the behaviour
   * demonstrated in the transcript above the system instruction. In a fresh
   * conversation the same input returned a list of ideas.
   *
   ...
   * (mobile's own task 1.4). Given "prices rise 5 percent from March, loyal
   * customers get 3 months notice", Qwen2.5 0.5B produced a draft asserting
   * "prices have increased by $100 per customer" — a figure with no source
   * in the input. Llama 3.2 1B, same prompt and input, invented nothing.
   *
...
export const MODES: readonly Mode[] = [
  ...
  },
  {
    ...
    // `connector_mode: 'required'` for exactly `id === 'search'`, which is
